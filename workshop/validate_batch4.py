#!/usr/bin/env python3
"""Validar Batch 4 - último setup para completar meta de 5"""
...
session_dir = Path(f"sessions/validation4_{session_id}")
session_dir.mkdir(parents=True, exist_ok=True)

print(f"\n{'='*80}")
print(f"🎯 VALIDAÇÃO BATCH 4 - Buscando último setup para completar 5/5!")
print(f"{'='*80}")
print(f"Progresso atual: 4/5 setups validados")
print(f"Meta: Encontrar 1 setup final com win rate >= 60%")
print(f"{'='*80}\n")

# 6 métodos finais: variações e métodos não testados
final_methods = [
    {"name": "vwap_trend_5m", "method": "vwap_trend", "tf": "5m"},  # vwap_trend teve 50% em 15m, testar 5m
    {"name": "trend_break_5m", "method": "trend_breakout", "tf": "5m"},  # trend_breakout teve 25% em 15m
    {"name": "stoch_cross_15m", "method": "stochastic_crossover", "tf": "15m"},  # Novo método
    {"name": "adx_trend_15m", "method": "adx_trend", "tf": "15m"},  # Novo método
    {"name": "supertrend_15m", "method": "supertrend", "tf": "15m"},  # Novo método
    {"name": "atr_break_15m", "method": "atr_breakout", "tf": "15m"},  # Novo método
]
...
print(f"📋 Batch 4: {len(final_methods)} métodos × {len(validation_periods)} períodos = {len(tests)} testes")
...
# Análise
print(f"\n{'='*80}")
print(f"📊 ANÁLISE BATCH 4 - ÚLTIMA BUSCA")
print(f"{'='*80}\n")
...
print(f"{'='*80}")
print(f"🎯 RESULTADO BATCH 4 - FINAL")
print(f"{'='*80}")
...
if validated_setups:
    ...
    if len(validated_setups) >= 1:
        print(f"\n🏆🏆🏆 META COMPLETA! 5/5 SETUPS VALIDADOS! 🏆🏆🏆")
else:
    print(f"\n⚠️  Nenhum novo setup validado. Ainda em 4/5.")
    print(f"Continuar testando mais métodos em Batch 5...")

# Salvar
with open(session_dir / "batch4_results.json", "w") as f:
    json.dump({"validated": validated_setups, "results": setup_results}, f, indent=2, default=str)

print(f"\n✅ Resultados salvos em: {session_dir / 'batch4_results.json'}")
print(f"{'='*80}\n")
