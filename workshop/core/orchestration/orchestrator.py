...
def start_remote_windows(ssh, sftp, args, remote_out, remote_zip, parquet_guess, remote_log, debug=False):
    ...
    cmd = (
        ...
        f'--v2_path {args.gpu_root}\\core\\selectors\\selector21.py '
...
if [ -f "{parquet_guess}" ]; then
  ...
  $PY {args.dl_script} \
    --v2_path "{args.gpu_root}/core/selectors/selector21.py" \
...
def legacy_main(argv=None):
    ...
    ap.add_argument("--v2_path", default="/opt/botscalp/core/selectors/selector21.py")
    ...
    cand = [
        args.selector_py,
        str(server_root / "selector7.py"),
        str(server_root / "core" / "selectors" / "selector21.py"),
    ]
    selector_py = None
    for c in cand:
