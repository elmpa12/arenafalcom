#!/usr/bin/env python3
"""Validar Batch 3 - mais 5 métodos para completar meta de 5 setups"""
...
session_dir = Path(f"sessions/validation3_{session_id}")
session_dir.mkdir(parents=True, exist_ok=True)

print(f"\n{'='*80}")
print(f"🎯 VALIDAÇÃO BATCH 3 - Buscando mais 3 setups")
print(f"{'='*80}")
print(f"Progresso atual: 2/5 setups validados")
print(f"Meta: Encontrar mais 3 setups com win rate >= 60%")
print(f"{'='*80}\n")

# 5 novos métodos + variações dos que funcionaram
new_methods = [
    {"name": "keltner_break_15m", "method": "keltner_breakout", "tf": "15m"},
    {"name": "donchian_break_15m", "method": "donchian_breakout", "tf": "15m"},
    {"name": "volume_break_15m", "method": "volume_breakout", "tf": "15m"},
    {"name": "pivot_break_15m", "method": "pivot_breakout", "tf": "15m"},
    {"name": "macd_trend_5m", "method": "macd_trend", "tf": "5m"},  # Testar MACD em 5m
    {"name": "ema_cross_15m", "method": "ema_crossover", "tf": "15m"},  # Testar EMA em 15m
]
...
print(f"📋 Batch 3: {len(new_methods)} métodos × {len(validation_periods)} períodos = {len(tests)} testes")
...
# Salvar
with open(session_dir / "batch3_results.json", "w") as f:
    json.dump({"validated": validated_setups, "results": setup_results}, f, indent=2, default=str)

print(f"\n✅ Resultados salvos em: {session_dir / 'batch3_results.json'}")
print(f"{'='*80}\n")
