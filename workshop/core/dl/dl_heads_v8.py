...
def main():
    ...
    # NPZ sequences (bypass enrichment/build)
    ap.add_argument("--seq_train", default="", help="NPZ com X/y/ret/time/(price) de treino (gera execução de uma única janela)")
    ap.add_argument("--seq_val", default="", help="NPZ com X/y/ret/time/(price) de validação")
    # Modelos
    ...
    # Dados: preferir NPZ quando especificados (única janela)
    X = Y = R = IDX = T = P = None
    if args.seq_train and args.seq_val:
        log("[DATA] Carregando NPZ de treino/validação")
        import numpy as np
        import pandas as pd
        ntr = np.load(args.seq_train)
        nva = np.load(args.seq_val)
        Xtr, Ytr = ntr["X"], ntr["y"]
        Xva, Yva = nva["X"], nva["y"]
        if "time" in nva.files:
            Tva = pd.to_datetime(nva["time"].astype("int64"), utc=True, unit="ms")
        else:
            Tva = pd.Series(pd.date_range("1970-01-01", periods=len(Yva), freq="T", tz="UTC"))
        if "price" in nva.files:
            Pva = pd.Series(nva["price"])  # opcional
        else:
            Pva = pd.Series(np.zeros(len(Yva), dtype=np.float32))
        # Caminho de janela única
        W = [(0, 0, 0, 0)]
        T = Tva; P = Pva
        feats = [f"f{i}" for i in range(Xtr.shape[-1])]
        log(f"[SEQ] NPZ: tr={len(Ytr)} va={len(Yva)} feats={len(feats)} lags={Xtr.shape[1]}")
    else:
        # Enriquecimento e sequências a partir do parquet
        if args.data_file:
